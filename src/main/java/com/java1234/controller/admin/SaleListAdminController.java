...
public class SaleListAdminController {
	...
	public Map<String,Object> save(SaleList saleList,String goodsJson)throws Exception{
		...
		Customer customer =customerService.findById(saleList.getCustomer().getId());
		if(Objects.nonNull(customer)){
			if ("是".equals(customer.getIshy())){
				customer.setJf(customer.getJf()+ 10);
				customerService.save(customer);
			}
		}

		saleListService.save(saleList, plgList);
		logService.save(new Log(Log.ADD_ACTION,"Add sales order")); 
		resultMap.put("success", true);	
		return resultMap;
	}
	...
	public Map<String,Object> delete(Integer id)throws Exception{
		...
		logService.save(new Log(Log.DELETE_ACTION,"Delete sales order"+saleListService.findById(id)));  // 写入日志
		resultMap.put("success", true);		
		return resultMap;
	}
